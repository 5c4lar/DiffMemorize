...
@click.option(
    "--window-size",
    help="Window size for dataloader sampler",
    metavar="FLOAT",
    type=click.FloatRange(min=0, max=1),
    default=0.5,
    show_default=True,
...
@click.option(
    "--num-channels",
    help="Number of network channels",
    metavar="INT",
    type=click.IntRange(min=1),
    default=128,
    show_default=True,
...
@click.option(
    "--embed",
    help="Time embedding",
    metavar="positional|fourier",
    type=click.Choice(["positional", "fourier"]),
    default="positional",
    show_default=True,
)
@click.option(
...
@click.option(
    "--ema_mode",
    help="How to adjust EMA ratio",
    metavar="mode1|mode2|mode3|mode4",
    type=click.Choice(["mode1", "mode2", "mode3", "mode4"]),
    default="mode2",
    show_default=True,
)
@click.option(
    "--skip-connections",
    help="Whether to enable each skip connection  [default: varies]",
    metavar="LIST",
    type=parse_int_list,
)
@click.option(
...
@click.option(
    "--wandb_group",
    help="wandb group used for logging",
    type=str,
    default=None,
    show_default=True,
)
def main(**kwargs):
    ...
    c.total_kimg = max(
        int(opts.duration * image_num / 50), 1
    )  # max(int(opts.duration * 1000 * opts.image_num / 50000), 1)
    c.ema_halflife_kimg = int(
        opts.ema * image_num / 50
    )  # int(opts.ema * 1000 * opts.image_num / 50000)
    ...
    c.update(
        batch_size=opts.batch, batch_gpu=opts.batch_gpu, window_size=opts.window_size
    )  # Add window_size
