...
def open_cifar10_balance(tarball: str, *, max_images: Optional[int], idx: Optional[Tuple[int]]):
    images = []
    labels = []
    ...
    for c in range(num_classes):
        ...
        balanced_images.append(images_class[idx * num_each_class : (idx + 1) * num_each_class])
        balanced_labels.append(labels_class[idx * num_each_class : (idx + 1) * num_each_class])
...
def open_dataset(source, *, max_images: Optional[int], idx: Optional[Tuple[int]]):
    if os.path.isdir(source):
        ...
        if os.path.basename(source) == "cifar-10-python.tar.gz":
            return open_cifar10_balance(source, max_images=max_images, idx=idx)
...
@click.option(
    "--idx",
    help="idx of slices to include in the dataset",
    metavar="INT",
    type=int,
)
def main(
    ...
    num_files, input_iter = open_dataset(source, max_images=max_images, idx=idx)
