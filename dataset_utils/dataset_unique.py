...
def open_cifar10_balance(
    tarball: str, *, max_images: Optional[int], num_classes: Optional[int]
):
    images = []
    labels = []
    ...
    max_idx = maybe_min(len(images), max_images)
    total_classes = 10
    num_each_class = max_idx // total_classes
    assert max_idx % total_classes == 0
    balanced_images = []
    balanced_labels = []
    for c in range(total_classes):
        labels_class = labels[labels == c]
        images_class = images[labels == c]
        print(images_class.shape)
        balanced_images.append(images_class[:num_each_class])
        balanced_labels.append(labels_class[:num_each_class])
    balanced_images = np.concatenate(balanced_images, axis=0)
    balanced_labels = np.concatenate(balanced_labels, axis=0)
    # # shuffle
    # shuffle_order = np.random.permutation(max_idx)
    # balanced_images = balanced_images[shuffle_order]
    # balanced_labels = balanced_labels[shuffle_order]
    ...
    def iterate_images():
        for idx, img in enumerate(balanced_images):
            yield dict(img=img, label=int(idx))
            if idx >= max_idx - 1:
                break
    ...
    *,
    ...
    if os.path.isdir(source):
        ...
        if os.path.basename(source) == "cifar-10-python.tar.gz":
            return open_cifar10_balance(
                source, max_images=max_images, num_classes=num_classes
            )
...
@click.option("--num-classes", help="Number of image classes", metavar="INT", type=int)
@click.option(
    "--transform",
