...
def open_cifar10_diversity(
    ...
    max_idx = maybe_min(len(images), max_images)
    # num_classes = 1
    num_each_class = max_idx // num_classes
    # assert max_idx % num_classes == 0
    balanced_images = []
    balanced_labels = []
    for c in range(num_classes):
        ...
        if c == num_classes - 1:
            balanced_images.append(
                images_class[: (max_idx - num_each_class * (num_classes - 1))]
            )
            balanced_labels.append(
                labels_class[: (max_idx - num_each_class * (num_classes - 1))]
            )
        else:
