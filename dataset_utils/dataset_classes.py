...
def open_cifar10_balance(tarball: str, *, max_images: Optional[int], classes: Optional[Tuple[int]]):
    images = []
    labels = []
...
def open_dataset(source, *, max_images: Optional[int], classes: Optional[Tuple[int]]):
    if os.path.isdir(source):
        ...
        if os.path.basename(source) == "cifar-10-python.tar.gz":
            return open_cifar10_balance(source, max_images=max_images, classes=classes)
...
@click.option(
    "--classes",
    help="list of classes to include in the dataset",
    metavar="INT",
    type=int,
    multiple=True,
)
def main(
    ...
    num_files, input_iter = open_dataset(source, max_images=max_images, classes=classes)
