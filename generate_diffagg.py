...
def logsumexp_tensor(a, axis=None, b=None, keep_dims=False, return_sign=False):
    device = a.device
    a = a.detach().cpu().numpy()
    if b is not None:
        b = b.detach().cpu().numpy()
    res = logsumexp(a, axis=axis, b=b, keepdims=keep_dims, return_sign=return_sign)
...
def diffagg_sampler(
    ...
    # Adjust noise levels based on what's supported by the network.
    sigma_min = max(sigma_min, nets[0].sigma_min)
    sigma_max = min(sigma_max, nets[0].sigma_max)
    ...
    t_steps = torch.cat(
        [nets[0].round_sigma(t_steps), torch.zeros_like(t_steps[:1])]
    )  # t_N = 0

    params, buffers = stack_module_state(nets)
    net = copy.deepcopy(nets[0])
    net = net.to('meta')
    weights = torch.tensor(weights, device=latents.device)
    
    def vmodel(params, buffers, y, t_hat, t_next,  i, c, epsilon):
        # t = t_hat
        # t_next = t_steps[i+1]   
        def step(y, t_hat, t_next, c):
            denoised = functional_call(net, (params, buffers), (y, t_hat, c)).to(torch.float64)
            d_cur = (y - denoised) / t_hat
            d = d_cur
            if i < num_steps - 1:
                y_next = y + (t_next - t_hat) * d_cur
                t_next = t_steps[i+1]
                denoised = functional_call(net, (params, buffers), (y_next, t_next, c)).to(torch.float64)
                d_prime = (y_next - denoised) / t_next
                d = (0.5 * d_cur + 0.5 * d_prime)
            return d
        v, vjpfunc = torch.func.vjp(
            lambda y: step(y, t_hat, t_next, c), y
        )
        vjps = vjpfunc(epsilon)
        trace = torch.einsum("bchw, bchw -> b", vjps[0], epsilon)
        v = step(y, t_hat, t_next, c)
        return v, trace
    vmodel_vmap = torch.vmap(vmodel, in_dims=(0, 0, None, None, None, None, None, None))
    # Main sampling loop.
    x_next = latents.to(torch.float64) * t_steps[0]
    x_nexts = torch.stack([x_next for _ in range(len(nets))])
    logpis = torch.zeros((len(nets), len(latents)), device=latents.device)
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):  # 0, ..., N-1
        x_cur = x_next
        x_curs = x_nexts
        # Increase noise temporarily.
        ...
        logpis = logpis_x - traces * (t_next - t_hat)
...
def main(
    ...
    **sampler_kwargs,
    ...
    for batch_seeds in tqdm.tqdm(
        ...
        latents = rnd.randn(
            [batch_size, nets[0].img_channels, nets[0].img_resolution, nets[0].img_resolution],
            device=device,
        )
        class_labels = None
        if nets[0].label_dim:
            class_labels = torch.eye(nets[0].label_dim, device=device)[
                rnd.randint(nets[0].label_dim, size=[batch_size], device=device)
            ]
        ...
        sampler_fn = diffagg_sampler
        images = sampler_fn(
            nets, weights, latents, class_labels, randn_like=rnd.randn_like, **sampler_kwargs
        )
