...
class SongUNet(torch.nn.Module):
    def __init__(self,
        ...
        skip_connections    = [],           # Whether to enable each skip connection, 0 or 1 for each element.
    ):
        ...
        assert len(skips) == len(self.skip_connections)
    ...
    def forward(self, x, noise_labels, class_labels, augment_labels=None):
        ...
        for name, block in self.enc.items():
            if 'aux_down' in name:
                ...
                if self.skip_connections[skip_num] == 1:
                    skips.append(x)
                else:
