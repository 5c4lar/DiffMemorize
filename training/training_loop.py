...
def training_loop(
    ...
    window_size=0.5,  # Window size in dataloader of sampler, if 0.0, enable full batch training mode.
    ...
    dist.print0(
        f"Training parameters: model_parameters: {total_params}, dataset_size: {len(dataset_obj)}, data_ratio: {data_ratio}, lr_rampup_kimg: {lr_rampup_kimg}, ema_halflife_kimg: {ema_halflife_kimg}, and ema mode: {ema_mode}"
    )
    dist.print0()
    ...
    while True:
        ...
        if ema_mode == "mode1":
            ema_beta = 0.5 ** (
                batch_size / max(ema_halflife_nimg, 1e-8)
            )  # schedule the ema according to data ratio
        elif ema_mode == "mode2":
            ema_beta = 0.5 ** (
                512 / max(ema_halflife_nimg, 1e-8) / data_ratio
            )  # compensate for the data ratio
        elif ema_mode == "mode3":
            ema_beta = 0.5 ** (batch_size / 500 / 1000)  # no schedule
        elif ema_mode == "mode4":
            ema_constant = 0.5 ** (batch_size / 500 / 1000)
            ema_beta = (
                final_ema
                / ema_constant
                * (0.5 ** (batch_size / max(ema_halflife_nimg, 1e-8) / data_ratio))
            )  # compensate for the data ratio
        else:
            raise AssertionError(
                f"Wrong ema_mode variable, got {ema_mode}, expected mode1 or mode2 or mode3 or mode4"
            )
        ...
        fields += [
            f"flop {training_stats.report0('FLOP/flop', flop_per_iter * (cur_nimg / batch_size)):<12.2e}"
        ]
        ...
        if dist.get_rank() == 0:
            ...
            if log_wandb:
                ...
                wandb.log(
                    {"flop": flop_per_iter * (cur_nimg // batch_size)},
