...
def training_loop(
    ...
    dataset_sampler = misc.InfiniteSampler(dataset=dataset_obj, rank=dist.get_rank(), num_replicas=dist.get_world_size(), seed=seed, window_size=window_size)
    ...
    while True:
        ...
        if ema_mode == "mode1":
            ema_beta = 0.5 ** (batch_size / max(ema_halflife_nimg, 1e-8))                # schedule the ema according to data ratio
        elif ema_mode == "mode2":
            ema_beta = 0.5 ** (512 / max(ema_halflife_nimg, 1e-8) / data_ratio)          # compensate for the data ratio
        elif ema_mode == "mode3":
            ema_beta = 0.5 ** (batch_size / 500 / 1000)                                  # no schedule
        elif ema_mode == "mode4":
            ema_constant = 0.5 ** (batch_size / 500 / 1000)
            ema_beta = final_ema / ema_constant * (0.5 ** (batch_size / max(ema_halflife_nimg, 1e-8) / data_ratio))   # compensate for the data ratio
        else:
            raise AssertionError(f'Wrong ema_mode variable, got {ema_mode}, expected mode1 or mode2 or mode3 or mode4')
