...
def calculate_knn_stats(
    ...
    for images, _ in tqdm.tqdm(
        ...
        distance = torch.cdist(
            images.reshape(N1, C * H * W), ref_images.reshape(N2, C * H * W)
        ) / np.sqrt(H * W * C)
        # distance, index = distance.min(dim=1)
        distance, index = torch.topk(
            distance, k=2, dim=1, largest=False
...
def calc_knn(image_path, ref_path, log_path, knn_save_path, num_expected, seed, batch):
    ...
    knn_distance, knn_index = calculate_knn_stats(
...
@click.option(
    "--expdir",
    help="Where to save all checkpoints",
    metavar="DIR",
    type=str,
    required=True,
)
@click.option(
    "--knn-ref",
    "knn_ref_path",
    help="Dataset reference statistics for KNN",
    metavar="NPZ|URL",
    type=str,
    required=True,
)
...
def main(
    ...
    **sampler_kwargs,
    ...
    network_pts = [
        os.path.join(expdir, pt) for pt in os.listdir(expdir) if pt.endswith(".pkl")
    ]
    network_pts.sort()
    ...
    for i in range(len(network_pts)):
        ...
        knn_save_path = os.path.join(
            knn_dir, os.path.basename(network_pkl).split(".")[0] + ".npz"
        )
        calc_knn(
            image_path=outdir,
            ref_path=knn_ref_path,
            log_path=log_path,
